export const onRequest = async (context: any) => {
  try {
    ...
    if (!file) {
      return new Response(JSON.stringify({ success: false, error: 'No file provided', code: 'BAD_REQUEST', url: '' }), { status: 400, headers: { 'Content-Type': 'application/json', ...cors } as any });
    }
    ...
    return new Response(
      ...
      { status: 500, headers: { 'Content-Type': 'application/json', ...cors } as any }
    );
  }
};
