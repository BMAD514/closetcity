export const onRequest: PagesFunction = async (context) => {
  try {
    ...
    if (!modelUrl || !garmentUrl || !poseKey) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing required fields: modelUrl, garmentUrl, poseKey', url: '', cached: false, code: 'BAD_REQUEST' }),
    ...
    return new Response(
      JSON.stringify({ success: false, error: error instanceof Error ? error.message : 'Try-on failed', url: '', cached: false, code: 'INTERNAL_ERROR' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
};
