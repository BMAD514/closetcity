export const onRequest = async (context: any) => {
  try {
    ...
    if (!userImageUrl) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing required field: userImageUrl', url: '', cached: false, code: 'BAD_REQUEST' }),
    ...
    return new Response(
      JSON.stringify({ success: false, error: error instanceof Error ? error.message : 'Model generation failed', url: '', cached: false, code: 'INTERNAL_ERROR' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
};
